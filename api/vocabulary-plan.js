...
export default async function handler(request, response) {
  ...
  const { language, level, count, topic, professionGroup, profession } = request.body || {}
  ...
  if ([language, level, topic, professionGroup, profession].some(value => typeof value === 'string' && value.length > 100)) return response.status(400).json({ error: 'Seçim bilgisi çok uzun.' })
